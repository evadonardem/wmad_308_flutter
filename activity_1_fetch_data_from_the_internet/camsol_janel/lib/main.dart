...
Future<Album> fetchAlbum(int albumId) async {
  final response = await http.get(Uri.parse("$endpoint/$albumId"));
    if (response.statusCode == 200) {
      print('Janel this is the data nang isang album lang');
      final rawData = response.body;
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Fetch data from the internet Janel.',
      theme: ThemeData(
        
        colorScheme: ColorScheme.fromSeed(seedColor: Color.fromARGB(255, 255, 174, 174)),
        useMaterial3: true,
      ),
      home: const MyHomePage(title: 'Fetch data from the internet Janel.'),
    );
  }
}
    ...
    return Scaffold(
      ...
      body: Center(      
        child: Row(
          children: [
         NavigationRail(
          backgroundColor: Color.fromARGB(255, 255, 174, 174),
          //extended: true,
          elevation: 5,
          destinations: [
          ...
          onDestinationSelected: (int index) {
            if (index !=2) {
              ...
              showAboutDialog(
                ...
                applicationVersion: 'MAY 2024 JANEL CAMSOL',
              );
            }
          },
          ),
