...
Future<Iterable<Album>> fetchAlbums() async {
  ...
  if (response.statusCode == 200) {
    print('Ebenezer eto ung data nung lahat ng album:');
    print(response.body);
...
Future fetchAlbum(int albumID)async{
  ...
  if (response.statusCode == 200) {
    print('Ebenezer eto ung data nung isang album:');
    final rawData = response.body;
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: Color.fromARGB(255, 5, 192, 176)),
        useMaterial3: true,
      ),
              ...
              onDestinationSelected: (int index){
                if(index != 2){
                  ...
                  showAboutDialog(
                    ...
                    applicationVersion: 'May 2024 || Kikilao, Ebenezer E.',
                  );
                }
...
class PhotosPage extends StatelessWidget{
    ...
    Widget build(BuildContext context){
      return Center(
        child: Text('Dito yung Photos page'),
      );
    }
}