...
Future<Iterable<Album>> fetchAlbums() async {
  ...
  if (response.statusCode == 200) {
      //print('this is the data nang lahat nang albums');
...
Future<Album> fetchAlbum(int albumId) async {
  final response = await http.get(Uri.parse("$endpoint/$albumId"));
    if (response.statusCode == 200) {
      print(' this is the data nang isang album lang');
      final rawData = response.body;
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        colorScheme: ColorScheme.fromSeed(seedColor: Color.fromARGB(255, 172, 6, 6)),
        useMaterial3: true,
      ),
      home: const MyHomePage(title: 'Fetch data from the internet .'),
    );
  }
}
    ...
    return Scaffold(
      ...
      body: Center(      
        child: Row(
          children: [
         NavigationRail(
          backgroundColor: Color.fromARGB(255, 44, 29, 24),
          //extended: true,
          elevation: 5,
          destinations: [
