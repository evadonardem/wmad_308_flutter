...
Future<Iterable<Album>> fetchAlbums() async {
  ...
  if (response.statusCode == 200) {
    print('eto po ung data nung lahat ng album:');
    print(response.body);
...
Future fetchAlbum(int albumID)async{
  ...
  if (response.statusCode == 200) {
    print('eto po ung data nung isang album:');
    final rawData = response.body;
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: Color.fromARGB(255, 231, 73, 165)),
        useMaterial3: true,
      ),
      home: const MyHomePage(title: 'Flutter Fetch Data from the Internet'),
    );
  }
}
              ...
              onDestinationSelected: (int index){
                if(index != 2){
                  ...
                  showAboutDialog(
                    ...
                    applicationVersion: 'May 2024 Tayaban Robinson',
                  );
                }
      ...
      return Center(
                child : FutureBuilder(future: albums, builder: ( context, snapshot) { 
                  if (snapshot.hasData) {
                   ...
                   final albumsCard = snapshot.requireData.map((album) => Card(child: ListTile(
