...
Future<Iterable<Album>> fetchAlbums() async {
  ...
  if (response.statusCode == 200) {
      //print('Jehanne this is the data nang lahat nang albums');
...
Future<Album> fetchAlbum(int albumId) async {
  final response = await http.get(Uri.parse("$endpoint/$albumId"));
    if (response.statusCode == 200) {
      print('Jehanne this is the data nang isang album lang');
      final rawData = response.body;
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Fetch data from the internet Jehanne.',
      theme: ThemeData(
        
        colorScheme: ColorScheme.fromSeed(seedColor: Color.fromARGB(255, 168, 11, 11)),
        useMaterial3: true,
      ),
      home: const MyHomePage(title: 'Fetch data from the internet Jehanne.'),
    );
  }
}
    ...
    return Scaffold(
      ...
      body: Center(      
        child: Row(
          children: [
         NavigationRail(
          backgroundColor: Color.fromARGB(255, 88, 28, 6),
          //extended: true,
          elevation: 5,
          destinations: [
          ...
          onDestinationSelected: (int index) {
            if (index !=2) {
              ...
              showAboutDialog(
                ...
                applicationVersion: 'MAY 2024 JEHANNE TOMAS',
              );
            }
          },
          ),
